...
// TestMountTrackerStateAfterRemove verifies mount tracker is cleared after Remove.
func TestMountTrackerStateAfterRemove(t *testing.T) {
	s := newTestSnapshotter(t)
	ctx := t.Context()

	// Create snapshot
	_, err := s.Prepare(ctx, "tracked-snap", "")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}

	// Get internal snapshotter to check tracker
	internal := s.(*snapshotter)

	// Manually set as mounted (simulating extract snapshot)
	// Note: We need to get the snapshot ID first
	var snapID string
	err = internal.ms.WithTransaction(ctx, false, func(txCtx context.Context) error {
		id, _, _, err := storage.GetInfo(txCtx, "tracked-snap")
		snapID = id
		return err
	})
	if err != nil {
	...
	internal.mountTracker.SetMounted(snapID)

	if !internal.mountTracker.IsMounted(snapID) {
		t.Fatal("snapshot should be marked as mounted")
	}

	// Remove snapshot
	if err := s.Remove(ctx, "tracked-snap"); err != nil {
		t.Fatalf("remove: %v", err)
	}

	// Mount state should be cleared
	if internal.mountTracker.IsMounted(snapID) {
		t.Error("mount state should be cleared after Remove")
	}
}
