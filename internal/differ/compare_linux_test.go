...
func TestRequiresMountResolution(t *testing.T) {
	tests := []struct {
		name  string
		lower []mount.Mount
		upper []mount.Mount
		...
		{
			name:  "empty mounts do not require resolution",
			lower: nil,
			upper: nil,
			want:  false,
		},
		{
			name:  "simple bind mounts do not require resolution",
			lower: []mount.Mount{{Type: "bind", Source: "/lower"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  false,
		},
		{
			name:  "mkfs/* upper requires resolution",
			lower: []mount.Mount{{Type: "bind", Source: "/lower"}},
			upper: []mount.Mount{{Type: "mkfs/ext4", Source: "/dev/sda1"}},
			want:  true,
		},
		{
			name:  "mkfs/erofs upper requires resolution",
			lower: nil,
			upper: []mount.Mount{{Type: "mkfs/erofs", Source: "/path/to/blob"}},
			want:  true,
		},
		{
			name:  "template in lower source requires resolution",
			lower: []mount.Mount{{Type: "bind", Source: "{{ mount 0 }}"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  true,
		},
		{
			name:  "template in upper source requires resolution",
			lower: []mount.Mount{{Type: "bind", Source: "/lower"}},
			upper: []mount.Mount{{Type: "bind", Source: "{{ mount 0 }}"}},
			want:  true,
		},
		{
			name:  "template in options requires resolution",
			lower: []mount.Mount{{Type: "overlay", Options: []string{"lowerdir={{ mount 0 }}"}}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  true,
		},
		{
			name:  "format/ type requires resolution",
			lower: []mount.Mount{{Type: "format/mkdir/overlay", Source: "/path"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  true,
		},
		{
			name:  "mkdir/ type requires resolution",
			lower: []mount.Mount{{Type: "mkdir/bind", Source: "/path"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  true,
		},
		{
		...
		},
		{
			...
			lower: []mount.Mount{{Type: "erofs", Source: "/path/to/layer.erofs"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  false,
		},
		{
			name: "overlay with static paths does not require resolution",
			lower: []mount.Mount{{
				Type:    "overlay",
				Options: []string{"lowerdir=/lower1:/lower2", "upperdir=/upper", "workdir=/work"},
			}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  false,
		},
		{
			name:  "format/mkfs/erofs in lower requires resolution",
			lower: []mount.Mount{{Type: "format/mkfs/erofs", Source: "/path"}},
			upper: []mount.Mount{{Type: "bind", Source: "/upper"}},
			want:  true,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := requiresMountResolution(tc.lower, tc.upper)
			if got != tc.want {
				t.Errorf("requiresMountResolution() = %v, want %v", got, tc.want)
			}
		})
	}
}
