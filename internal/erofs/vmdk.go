...
// WriteVMDKDescriptor writes a VMDK descriptor to the provided writer.
// The descriptor references the given device files as extents, allowing
// QEMU to present them as a single concatenated block device.
func WriteVMDKDescriptor(w io.Writer, devices []string) error {
	cid := uint32(0xfffffffe)
...
// WriteVMDKDescriptorToFile writes a VMDK descriptor to the given file path.
func WriteVMDKDescriptorToFile(vmdkPath string, devices []string) error {
	f, err := os.Create(vmdkPath)
	if err != nil {
		return err
	}
	if err := WriteVMDKDescriptor(f, devices); err != nil {
		_ = f.Close()
		return err
	}
	return f.Close()
